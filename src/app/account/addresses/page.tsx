...
import { setData } from "@/services/operations";
...
export default function AddressesPage() {
  ...
  const [formAddress, setFormAddress] = useState<Partial<IAddress>>({
    type: "Home",
    addressLine1: "",
    city: "",
    state: "",
  ...
  const handleSetDefault = (id: string) => {
    ...
    setData("users", user.uid, user).then(() =>
      toast({
        ...
        description: "The default shipping address has been changed.",
      })
    );
  };
  ...
  const handleSaveAddress = () => {
    ...
    if (editingAddress) {
      ...
      const newUser = {
        ...user,
        addresses: user.addresses.map((addr) =>
          addr.uid === editingAddress.uid
            ? ({ ...addr, ...formAddress } as IAddress)
            : addr
        ),
      };
      setUser(newUser);
              ...
              {!address.isDefault && (
                <CardFooter>
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => handleSetDefault(address.uid)}
